...
RSpec.describe GeoStats::LiquidAggregateTag do
  ...
  before do
    ...
    allow(GeoStats::QueryAggregator).to receive(:aggregate).and_return(agg_result)
    allow(GeoStats::QueryAggregator).to receive(:breakdown).and_return(breakdown_result)
  end

  def build_tag(markup)
    described_class.new('geo_aggregate', markup, [])
  end
  ...
  describe 'scope resolution from issues drop' do
    ...
    it 'extracts @issues ivar from the drop and runs aggregation' do
      ...
      tag = build_tag('from: issues, periods: 6, closed_statuses: "Closed;Rejected", assign_to: geo')

      expect(GeoStats::QueryAggregator).to receive(:aggregate).with(scope, anything).and_return(agg_result)
    ...
    it 'assigns result to the named variable in context' do
      ctx = build_context('issues' => drop_with_ivar)
      tag = build_tag('from: issues, assign_to: geo')

      tag.render(ctx)

      expect(ctx.scopes.last['geo']).to eq(agg_result)
    end

    it 'defaults assign_to to "geo" when omitted' do
      ctx = build_context('issues' => drop_with_ivar)
  ...
  describe 'scope resolution from query_id' do
    ...
    it 'finds the IssueQuery by id and uses base_scope' do
      ctx = build_context
      tag = build_tag('query_id: 42, periods: 6, assign_to: geo')
    ...
    it 'resolves query_id from Liquid context when it is a variable' do
      ...
      tag = build_tag('query_id: my_qid, assign_to: geo')
    ...
    it 'assigns empty result when query_id is not found' do
      ctx = build_context
      tag = build_tag('query_id: 999, assign_to: geo')

      tag.render(ctx)

      expect(ctx.scopes.last['geo']['total']).to eq(0)
    end
  end
  ...
  describe 'scope resolution from context registers' do
    it 'uses :geo_issue_query register when present (Reporter patch path)' do
      query = LiquidTagIssueQueryStub.new(scope)
      ctx = build_context({}, { geo_issue_query: query })
      tag = build_tag('from: issues, assign_to: geo')
    ...
    it 'uses :controller register @query when :geo_issue_query and :container absent' do
      query      = LiquidTagIssueQueryStub.new(scope)
    ...
    it 'prefers :geo_issue_query over :container and :controller' do
      ...
      ctx = build_context({}, { geo_issue_query: query1, container: query2, controller: controller })
      tag = build_tag('from: issues, assign_to: geo')
  ...
  # ------------------------------------------------------------------
  # Scope resolution via drop ivar inspection (@geo_base_scope patch path)
  # ------------------------------------------------------------------

  describe 'scope resolution from drop @geo_base_scope ivar' do
    it 'uses @geo_base_scope ivar when present on the drop (Strategy A patch)' do
      drop = Object.new
      drop.instance_variable_set(:@issues, [double('issue', id: 1)])
      drop.instance_variable_set(:@geo_base_scope, scope)
      ctx = build_context('issues' => drop)
      tag = build_tag('from: issues, assign_to: geo')
  ...
  describe 'parameter parsing' do
    ...
    it 'parses period as a string' do
      ctx = build_context('issues' => drop)
      tag = build_tag('from: issues, period: week, assign_to: geo')

      expect(GeoStats::QueryAggregator).to receive(:aggregate)
    ...
    it 'passes period: day correctly' do
      ctx = build_context('issues' => drop)
      tag = build_tag('from: issues, period: day, periods: 30, assign_to: geo')
    ...
    it 'passes period: year correctly' do
      ctx = build_context('issues' => drop)
      tag = build_tag('from: issues, period: year, periods: 3, assign_to: geo')
    ...
    it 'parses periods as an integer' do
      ctx = build_context('issues' => drop)
      tag = build_tag('from: issues, period: month, periods: 12, assign_to: geo')
    ...
    it 'passes nil periods when omitted (letting aggregator use default)' do
      ctx = build_context('issues' => drop)
      tag = build_tag('from: issues, period: month, assign_to: geo')
    ...
    it 'accepts legacy months param as alias for periods when period is month' do
      ctx = build_context('issues' => drop)
      tag = build_tag('from: issues, months: 12, assign_to: geo')
    ...
    it 'parses closed_statuses from double-quoted string' do
      ctx = build_context('issues' => drop)
      tag = build_tag('from: issues, closed_statuses: "Closed;Rejected", assign_to: geo')
    ...
    it 'parses closed_statuses from single-quoted string' do
      ctx = build_context('issues' => drop)
      tag = build_tag("from: issues, closed_statuses: 'Closed,Done', assign_to: geo")
  ...
  describe 'breakdown mode' do
    ...
    it 'calls QueryAggregator.breakdown when group_by is present' do
      ctx = build_context('issues' => drop)
      tag = build_tag('from: issues, group_by: tracker, assign_to: geo')

      expect(GeoStats::QueryAggregator).to receive(:breakdown)
    ...
    it 'does NOT call aggregate when group_by is present' do
      ctx = build_context('issues' => drop)
      tag = build_tag('from: issues, group_by: status, assign_to: geo')

      expect(GeoStats::QueryAggregator).not_to receive(:aggregate)
      expect(GeoStats::QueryAggregator).to receive(:breakdown).and_return(breakdown_result)
    ...
    it 'passes group_by: priority correctly' do
      ctx = build_context('issues' => drop)
      tag = build_tag('from: issues, group_by: priority, assign_to: geo')
    ...
    it 'passes group_by: assignee correctly' do
      ctx = build_context('issues' => drop)
      tag = build_tag('from: issues, group_by: assignee, assign_to: geo')
    ...
    it 'uses aggregate (not breakdown) when group_by is absent' do
      ctx = build_context('issues' => drop)
      tag = build_tag('from: issues, period: month, periods: 6, assign_to: geo')

      expect(GeoStats::QueryAggregator).to receive(:aggregate).and_return(agg_result)
      expect(GeoStats::QueryAggregator).not_to receive(:breakdown)

      tag.render(ctx)
    end

    it 'assigns empty result and returns blank when breakdown raises' do
      allow(GeoStats::QueryAggregator).to receive(:breakdown).and_raise(StandardError, 'oops')
    ...
    it 'empty_result includes buckets key for safe template access' do
      ...
      expect(ctx.scopes.last['geo']).to have_key('buckets')
    end
  end
  ...
  describe 'error handling' do
    ...
    it 'assigns empty result and returns blank when aggregator raises' do
      drop = Object.new
      drop.instance_variable_set(:@issues, scope)
      allow(GeoStats::QueryAggregator).to receive(:aggregate).and_raise(StandardError, 'db error')
