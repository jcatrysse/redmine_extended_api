...
module GeoStats
  ...
  #   {% geo_aggregate from: issues, period: month, periods: 6,
  #      closed_statuses: "Closed;Rejected", assign_to: geo %}
  #
  # Usage (after reporter plugin fix exposes query_id):
  #   {% geo_aggregate query_id: query_id, period: week, periods: 13,
  #      closed_statuses: "Closed;Rejected", assign_to: geo %}
  #
  ...
  class LiquidAggregateTag < Liquid::Tag
    ...
    def render(context)
      ...
      assign_to = str_param(@raw_params['assign_to'], context, default: 'geo')
      t0        = Process.clock_gettime(Process::CLOCK_MONOTONIC)
      scope     = resolve_scope(context)

      if scope.nil?
        Rails.logger.warn('[geo_aggregate] could not resolve an AR scope — skipping aggregation')
        context.scopes.last[assign_to] = empty_result
        return ''
      end

      Rails.logger.info("[geo_aggregate] scope resolved via #{scope_class_label(scope)} in #{elapsed_ms(t0)}ms")
      t1 = Process.clock_gettime(Process::CLOCK_MONOTONIC)

      result = if @raw_params.key?('group_by')
        group_by = str_param(@raw_params['group_by'], context, default: 'status')
        GeoStats::QueryAggregator.breakdown(scope, group_by: group_by)
      else
        ...
        GeoStats::QueryAggregator.aggregate(
          scope,
          period:          period,
          periods:         periods,
      ...
      Rails.logger.info("[geo_aggregate] SQL aggregation done in #{elapsed_ms(t1)}ms (total #{elapsed_ms(t0)}ms)")
      context.scopes.last[assign_to] = result
      ''
    rescue => e
      Rails.logger.error("[geo_aggregate] #{e.class}: #{e.message}\n#{e.backtrace.first(5).join("\n")}")
      context.scopes.last[assign_to] = empty_result
      ''
    end
    ...
    #
    # Priority:
    #   1. :geo_issue_query — set by geo_reporter_render_patch.rb once activated
    ...
    def scope_from_query_id(param, context)
      ...
      Rails.logger.warn("[geo_aggregate] query_id lookup failed: #{e.message}")
      nil
    end
    ...
    def scope_from_drop(drop)
      ...
      # geo_reporter_render_patch.rb sets @geo_base_scope on the drop when
