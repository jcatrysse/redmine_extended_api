...
#     → geo_aggregate reconstructs Issue.where(id: [10k ids]) — slow
#
# With this patch:
#   generate_reports(scope, query_id)
#     → liquidize(scope) stores an AR::Relation in IssuesDrop#@issues
#     → geo_aggregate uses the scope directly for COUNT(*) GROUP BY — fast
  ...
  def generate_reports(issues, query_id = nil)
    if query_id.present?
      begin
        ...
        if scope
          Rails.logger.info("[geo_reporter_list_patch] using base_scope for query #{query_id}")
          html = liquidize(scope)
          return [Report.new(name, filename, html, public_link_params(issues, query_id), orientation)]
        end
      rescue => e
        Rails.logger.warn("[geo_reporter_list_patch] #{e.class}: #{e.message} — falling back to full load")
      end
    end
    super
  end
